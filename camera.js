...
var videoRecorder = class videoRecorder {
  ...
  init() {
    ...
    if (self.selectVideoSources === null) {
      console.error(
        "Needed a select/input to append video sources. not configured a required one"
      );
      foundNullDom = true;
    }
    if (self.selectAudioSources === null) {
      console.error(
        "Needed a select/input to append audio sources. not configured a required one"
      );
      foundNullDom = true;
    }
    if (self.selectVideoBitrate === null) {
      console.error(
        "Needed a select/input to append video bitrate. not configured a required one"
      );
      foundNullDom = true;
    }
    if (self.selectAudioBitrate === null) {
      console.error(
        "Needed a select/input to append audio bitrate. not configured a required one"
      );
      foundNullDom = true;
    }
    ...
    self.selectAudioSources.onchange = function () {
      self.getStream(self);
    };
    self.selectVideoSources.onchange = function () {
      self.getStream(self);
    };
    self.recordButton.onclick = function () {
      self.toggleRecording(self);
    };
    self.downloadButton.onclick = function () {
      self.download(self);
    };
    self.previewButton.onclick = function () {
      self.togglePreview(self);
    };
    self.recordedVideo.addEventListener(
      "error",
      function (ev) {
            ...
            $(self.selectVideoSources)
              ...
              .text(
                videodevices[i].label ||
                  "camera " + (self.selectVideoSources.length + 1)
              );
          } else {
            $(
              `<option value="${videodevices[i].deviceId}">${
        ...
        for (let i = 0; i !== audiodevices.length; ++i) {
          if (
            ...
            $(self.selectAudioSources)
              ...
              .text(
                audiodevices[i].label ||
                  "camera " + (self.selectAudioSources.length + 1)
              );
          } else {
            $(
              `<option value="${audiodevices[i].deviceId}">${
            ...
            ).appendTo($(self.selectAudioSources));
          }
        }
        // now we set a default empty value for the selects
        if (
          $(self.selectVideoSources).find(`option[value='${self.nullDevice}']`)
            .length === 0
        ) {
          $(`<option value="${self.nullDevice}">None</option>`).appendTo(
            $(self.selectVideoSources)
          );
          $(
  ...
  getStream(self) {
    ...
    function testAndRun(self, currentResolution) {
      const cameraname =
        self.selectVideoSources.options[
          self.selectVideoSources.options.selectedIndex
        ].text;
      try {
        var constrains = {
          audio: {
          ...
          video: {
            ...
            height: {
              exact: self.requiredResolutions[currentResolution].height,
            },
        ...
        if (self.logging)
          console.log(
            `${cameraname}: Resolution set to ${self.requiredResolutions[currentResolution].width} x ${self.requiredResolutions[currentResolution].height}`
          );
  ...
  startRecording(self) {
    ...
    if (!MediaRecorder.isTypeSupported(self.recorderOptions.mimeType)) {
      if (self.logging)
        console.log(self.recorderOptions.mimeType + " is not Supported");
      ...
      if (!MediaRecorder.isTypeSupported(self.recorderOptions.mimeType)) {
        ...
        if (!MediaRecorder.isTypeSupported(self.recorderOptions.mimeType)) {
          ...
          if (!MediaRecorder.isTypeSupported(self.recorderOptions.mimeType)) {
            ...
            console.warn(
              "Not able to find a suitable container. mimeType will be empty"
            );
          }
        }
      }
    ...
    try {
      self.recorderOptions.audioBitsPerSecond = parseInt(
        self.selectAudioBitrate.value,
        10
      );
      self.recorderOptions.videoBitsPerSecond = parseInt(
        self.selectVideoBitrate.value,
        10
      );
      ...
      alert(
        `Exception while creating MediaRecorder: ${e}.mimeType: ${self.recorderOptions.mimeType}`
      );
      return;
    }
  }
